...
def main():
    ...
    for rel_path in TEST_FILES:
        ...
        print(f"{'Channel Name':30s}  {'display_fmt':>11s}  {'unit_type':>10s}  {'cfg_flags':>10s}  {'Units':>8s}  {'Function'}")
        print("-" * 120)

        for c in file_channels:
            fn = c['function'] if c['function'] else '** EMPTY **'
            print(f"{c['long_name']:30s}  {c['display_format']:>11d}  0x{c['unit_type_byte']:02x}        0x{c['config_flags']:04x}      {c['units']:>8s}  {fn}")
    ...
    for (df, utb), channels in sorted(by_triple.items()):
        ...
        print(f"  (display_format={df}, unit_type_byte=0x{utb:02x})  units=\"{units}\"  config_flags={[f'0x{f:04x}' for f in unique_cfg_flags]}")
        print(f"    Appears in {len(channels)} channel instances across {len(unique_files)} file(s)")
        print(f"    Channel names ({len(unique_names)}):")
        for name in unique_names:
            # Find all files this name+triple appears in
            name_files = sorted(set(c['file'].split('/')[-1] for c in channels if c['long_name'] == name))
    ...
    print(f"{'Channel Name':30s}  {'df':>3s}  {'utb':>5s}  {'cfg':>6s}  {'units':>6s}  "
    ...
    for c in sorted(empty_channels, key=lambda x: (x['display_format'], x['unit_type_byte'], x['long_name'])):
        ...
        print(f"{c['long_name']:30s}  {c['display_format']:>3d}  0x{c['unit_type_byte']:02x}  0x{c['config_flags']:04x}  {c['units']:>6s}  "
    ...
    for (df, utb), channels in sorted(by_triple.items()):
        ...
        print(f"    unit_type_7bit={unit_type_7bit} (0x{unit_type_7bit:02x}), calibrated_flag={calibrated}")
    ...
    print("=" * 130)
    print()
    print(f"{'(df, utb)':>15s}  {'unit_7bit':>8s}  {'cal':>3s}  {'units':>8s}  {'#ch':>4s}  {'#files':>6s}  {'config_flags':>30s}  {'Channel Names'}")
    print("-" * 160)
    ...
    for (df, utb), channels in sorted(by_triple.items()):
        ...
        print(f"({df:>3d}, 0x{utb:02x})      0x{unit_type_7bit:02x}     {calibrated:>3d}  {units:>8s}  {len(channels):>4d}  {unique_files:>6d}  {cfg_str:>30s}  {names_str}")
