...
def main():
    ...
    for field_name, n_unique, mi, nmi, determines in sorted(single_results, key=lambda x: -x[3]):
    ...
    for field_name in candidate_fields:
        ...
        varying = {name: vals for name, vals in by_name.items() if len(vals) > 1}
        if varying:
            print(f"  {field_name}: {len(varying)} channels vary across files")
    ...
    print("=" * 120)
    print(
        "RAW HEX FOR BYTES [6:16] (source_channel_id, hw_ref, unit_type, display_fmt, config_flags, source_type)"
    )
    print("=" * 120)
    print()
    for f in sorted(all_fields_86, key=lambda x: x["long_name"]):
        ...
        print(f"  {f['long_name']:25s}  [6:16]={hex_6_16}  " f"[76:96]={hex_76_96}  fn={fn}")
